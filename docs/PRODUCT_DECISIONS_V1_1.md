...
16. Classroom operation must be internet-independent after installation on the school LAN. Hosted public service is tactical/demo fallback only.
...
19. Publisher-operated Vercel demos are authorized. Public demo: 30-minute wall, 10-minute idle, 15-minute read-only debrief. Offline evaluation: 14 days. Classroom and agency-training Windows pilots: 90 days. Classes: 30-180 minutes, 60 default. Expiry does not delete records or interrupt monitoring/recovery/logging/export.
20. Current work remains on `codex/v1.1-closure`. The release candidate must be a clean reviewed commit on `codex/release-v1.1`, tagged `v1.1.0-rc.1`, with signed/promoted artifacts proving the exact revision.
