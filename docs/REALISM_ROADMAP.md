...
| WP-3/4 | **DONE — catalog terrain coverage** | 13 physical DEM packages (Terrarium/3DEP, `surface: dtm-approx`), one per realism AO: `demo_wildfire` and all 10 `hist_*` disaster AOs plus the two `train_*` terrain AOs. **Overture building footprints now on 6 AOs** — `demo_wildfire`, Surfside, and the urban disaster cores Katrina (New Orleans), Harvey (Houston) and the Marshall Fire suburbs (the two densest cropped to the mission core for §21's 250 KB footprint budget; Marshall fits whole). Exact LOS, AGL/MSL surface safety, provenance and fixture-budget enforcement; every route verified in-bounds and above terrain by `scenarioRouteSanity`/`determinism`. |
| WP-7 GNSS | **DONE + LIVE (catalog-wide)** | Real CelesTrak YUMA almanac → IS-GPS-200 propagation → frozen az/el → `skyVisibility()` → DOP → σ_H → reported position. Loss of fix on <4 sats or HDOP > 20. Truth retained; error is band-limited noise from sim time, so no RNG state and replay stays byte-identical. **Now on all 13 terrain AOs** (was `demo_wildfire` only): every AO with a committed DEM carries a constellation, so occlusion has real geometry to mask everywhere the model runs; `gnssCoverage.spec` proves masking genuinely occurs across the relief AOs and never fabricates a satellite. Representative recent almanac (terrain masking is the training content, not the incident-date sky). |
...
## WP-8 · RF link budget `[Tranche C]` `[needs WP-4]` — **DONE + LIVE**

**Status:** complete. `src/sim/safety/commsModel.ts` implements §18.4's log-distance model with
per-scenario clutter exponents, true LOS/NLOS from WP-4, seeded shadow fading and a one-hop relay
solver; `applyCommsModel` now computes `signalDbm` from geometry rather than integrating a ramp.

**The authored inputs were demoted, not deleted.** Comms-loss windows and weather reliability
become **impairments in dB** added to the path loss like any other attenuation. A blackout window
still costs real margin — the wildfire scenario's smoke/RF event at T+80s is a genuine authored
event — but it can no longer force a link down on its own, and an aircraft parked near the
operator rides it out. Placement decides.

**⚠ One §18.4 term needed a stated modelling choice.** The table's exponents describe
*ground-level* links, where the ray runs through rooftops. Applying them unmodified to an
air-to-ground link is wrong in a way that matters: a drone at 60 m and 2 km out subtends 1.7°,
which reads as a grazing ground path, but the first Fresnel radius there is ~8 m against ~50 m of
rooftop clearance — the link is essentially free space. Scoring it by the raw exponent imposes
~30 dB of clutter loss that physically is not there and makes most of the catalog unflyable.

`effectiveExponent` therefore blends from the clutter exponent toward free-space 2.0 by the
aircraft's **height above the clutter top** (a new `clutterHeightM` per class: 2/6/9/20/40 m).
Elevation angle was tried first and rejected — it fails exactly at the long-range, low-angle case
that dominates these missions. Terrain and individual structures are still handled exactly, by
the WP-4 NLOS term; this governs only the statistical clutter the DEM does not resolve.

**Radio spec.** A generic 2.4 GHz public-safety C2 link — 27 dBm, 2 dBi airframe omni, 8 dBi
ground patch, −101 dBm sensitivity — chosen to close a free-space link at ~10 km, the right order
for the multi-kilometre range these platforms advertise. Not from any one vendor's datasheet.

**Original WP-8 target below (unchanged).**

**Current state (before this work):** seeded comms-loss windows plus a weather reliability factor.
Scripted — the operator cannot influence it, so it teaches endurance rather than decision-making.
...
**Files (as built):** `src/sim/safety/commsModel.ts` (new) · `src/sim/safety/SafetyManager.ts` ·
`src/types/index.ts` (`rfClutter` on `ScenarioConfig`; link fields on `DroneState`) ·
`src/components/FleetPanel.tsx` (LINK readout: margin, NLOS, relay hop).
Clutter class is **derived from each scenario's existing `weatherProfile.locationTag`** rather
than re-authored across all 21, with explicit `rfClutter: 'dense_urban'` overrides where that tag
is too coarse — the Financial District, Times Square and the SF core.

**Accept — all met:** moving a relay drone measurably changes downstream link margin (and the
optimum is genuinely mid-path, not "as far out as possible"); a drone behind a ridge loses margin
with `interferenceDb === 0`, i.e. without any scripted event; the existing comms-loss tests are
rewritten against the physical model.

**Test:** `commsModel.spec.ts` (20) — free-space anchors (40 dB @ 1 m, 100 dB @ 1 km, 6 dB per
doubling), §18.4 exponent ordering, the height-above-clutter blend, NLOS penalty inside the
published 15–25 dB band and scaled by blocker depth, bounded zero-mean spatially-correlated
shadow fading, the margin → packet-loss → latency curve, relay selection and repositioning, and
statelessness (feeding the model its own output changes nothing) ·
`commsModelUrban.spec.ts` (4, rewritten) — signal is a function of geometry rather than of
elapsed ticks, weather stays bounded, and a blackout window is an impairment rather than an
override.
