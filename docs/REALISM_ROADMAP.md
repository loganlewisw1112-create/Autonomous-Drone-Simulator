...
| WP-3/4 | **DONE — catalog terrain coverage** | 13 physical DEM packages (Terrarium/3DEP, `surface: dtm-approx`), one per realism AO: `demo_wildfire` (+ Overture buildings) and all 10 `hist_*` disaster AOs plus the two `train_*` terrain AOs. Exact LOS, AGL/MSL surface safety, provenance and fixture-budget enforcement; every route verified in-bounds and above terrain by `scenarioRouteSanity`/`determinism`. |
