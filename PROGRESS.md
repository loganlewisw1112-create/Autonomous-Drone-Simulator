...
## State
Current phase: **COMPLETE — `GATE 6 PASS assertions=11/11`.** The plan ran end to end; the Gate 6 abort on 6.1 was
resolved by the owner (see `ABORT.md` and "Owner-authorized fixes" below).
Gate status: Gate 6 = 11/11. Every prior gate passes both inside Gate 6.6 and individually: P0 17/17, 0 9/9, 1 8/8,
2 10/10, **3 10/10 (3.4 un-skipped, building shadows fixed)**, 4 9/9, 5 8/8. No baseline regression (6.4: 1272 tests).
...
- [x] Phase 6 quality + verification - `a54d197` - `GATE 6 FAIL assertions=10/11 failed=[6.1] p75_layer_ms=0.9` **V**. Aborted on 6.1 per plan Part 5; `ABORT.md` had the evidence and recommendation. (Superseded by the owner-authorized fixes below, which take Gate 6 to `PASS 11/11`.)
- [x] **Owner-authorized fixes (2026-09-21)** - `GATE 6 PASS assertions=11/11 failed=[] p75_layer_ms=1.1` **V** (`node harness/gates/run.mjs 6`). Four owner decisions actioned; see the section below and the updated Gate 6 record.

## Gate 6 record
| # | Result **V** |
|---|---|
| **6.1 frame budget (OWNER-ADOPTED 2026-09-21)** | **PASS.** Gates the LAYER's own attributable cost, not whole-frame time (owner adopted `ABORT.md` rec #2). Layer `render()` p75 worst **1.1 ms** (budget 8); median frame time added over the map alone **0.2 ms** (budget 4), p90 5.6 ms. Whole-frame AS WRITTEN still measured and printed: 0/72 <= 16.7 ms, GPU-bound by the map — not the gated budget. See `ABORT.md`. |
| 6.1b (diagnostic) the layer is never the cause | PASS. Every one of the 72 cells over the 16.7 ms whole-frame budget is over it with the 3D layer REMOVED too — the cost is the map, not the layer |
...
| 6.6 all prior gates | PASS. P0 17/17, 0 9/9, 1 8/8, 2 10/10, **3 10/10 (3.4 un-skipped, passing)**, 4 9/9, 5 8/8. One retry per gate absorbs this iGPU's transient tile stalls under back-to-back load (a real regression fails both attempts); every prior gate also verified passing individually |
...
## Owner-authorized fixes (2026-09-21)
After the abort, the owner reviewed the four open decisions and authorized all four fixes. All are done; `GATE 6 PASS 11/11` and every prior gate passes. Nothing pushed.

1. **#1 Frame budget — adopt the layer-attributable budget** (`harness/gates/gate-6.mjs`). Owner chose `ABORT.md` rec #2: gate the layer's own cost, not whole-frame time (GPU-bound by the map on this iGPU). 6.1 now checks layer `render()` p75 <= 8 ms per cell AND median added frame time <= 4 ms (measured worst 1.1 ms, median added 0.2 ms). The whole-frame numbers are still measured and printed; 6.1b proves the layer is never the cause of a miss. A dated owner decision, not a silent loosening — see `ABORT.md` (RESOLVED).
2. **#2 Building shadows — fixed** (`shadowReceivers.ts`, on by default via `mount.ts`, doc in `index.ts`). The old single mesh both cast and received, so every roof sat in its own shadow (the 3.4 defect). Split into a CASTER (walls + cap at the real roof height, invisible/depth-only, receives nothing) and a RECEIVER (roof plane lifted `ROOF_CLEARANCE_M` = 1.5 m above the cap, receive-only). Separate meshes at different depths → a roof can never self-shadow. Enabled by default in the product mount (no-op where a scenario has no footprints). Gate 3.4 **un-skipped** and now passes (`--with-buildings` kept as a no-op alias); FALLBACK-3 rescinded.
3. **#3 Terrain coverage — fixed** (`scenarioTerrainLayers.impl.ts` [owner-authorized out-of-plan file], `terrainModel.ts`). `extractTile` dropped any DEM tile straddling the crop edge, so drawn relief stopped up to a full tile (256 px) short of the DEM's real bounds on every side. It now serves those edge tiles edge-clamped (the same clamp `elevationAt()` applies), so relief reaches the crop bounds. `terrainModel` widened from `wholeTileBounds` to `cropBounds` so aircraft/shadows sit correctly over the newly-drawn edges; `wholeTileBounds` kept for the harness ridge search.
...
| 3.4 climbs buildings | **PASS (fixed 2026-09-21, un-skipped).** Over a 5.6 m Overture footprint the aircraft shadow lands **9.4 px** from the roof-plane prediction, 42.7 px from where a ground shadow would fall (they are 33 px apart); 338 shadow px. Caster/receiver split in `shadowReceivers.ts` — see Owner-authorized fixes |
...
- **Owner-authorized out-of-plan file (2026-09-21): `src/components/scenarioTerrainLayers.impl.ts`.** The #3 terrain-coverage fix edits one existing app file the plan reserved for the owner's explicit call; the owner authorized it. Added to the P0.1c declared-paths allowlist (`gate-p0.mjs`) with a comment naming it an owner-authorized deviation.
- **Harness robustness edits (2026-09-21):** `harness/probe.mjs` Chrome anti-throttling flags; `gate-6.mjs` 6.6 one-retry-per-gate with cooldown. Both address this iGPU's transient tile/rAF flakiness under sustained headed-browser load; neither changes any assertion (a real regression still fails). See Owner-authorized fixes.
...
- **FALLBACK 3 (buildings) — RESCINDED 2026-09-21.** Building shadows are fixed (caster/receiver split; see Owner-authorized fixes), on by default, and Gate 3.4 is un-skipped and passing. The trail below is kept for the record. Original note: **FALLBACK 3 taken NARROWLY - for buildings only.** The plan's fallback is all-or-nothing (drop the receiver mesh for blob decals; mark 3.3-3.6 N/A). But only 3.4 failed: the terrain receiver passed 3.1-3.3, 3.5-3.7 on its first run. Replacing a verified terrain receiver with blobs would be a downgrade made only to follow the letter, so the fallback is applied to the failing part alone: building stand-ins ship **OFF by default**, 3.4 is reported `[SKIP]` (never counted as a pass), and everything else stays a real, measured pass - strictly more verified than the plan's own fallback. No criterion was loosened. The 2-attempt limit was honoured:
...
- The night-darkening and "white background" are addressed by the grade (#4, resolved above). Fog-band weight at 4.8 km and the exact grade tint strengths remain **taste items for the ongoing aesthetic pass** the owner is steering — simple knobs (`atmosphere.ts` grade alphas; MapLibre fog terms), not gate failures.
...
- ~~**Building shadow stand-ins self-shadow (Gate 3.4)**~~ **RESOLVED 2026-09-21 (owner-authorized #2).** Fixed by the caster/receiver split (`shadowReceivers.ts`); on by default, Gate 3.4 un-skipped and passing.
...
- ~~**The basemap does not get dark at night.**~~ **RESOLVED 2026-09-21 (owner-authorized #4).** The atmosphere GRADE (multiply wash keyed to palette darkness) darkens/tints the whole view by time of day, so the night basemap reads dark and noon stops reading white. Original note: Phase 2 lights the MODELS; MapLibre's 2D style stayed daytime-bright, so a dark night airframe sat on a bright map. (Fine tuning of the tint is part of the ongoing aesthetic pass.)
...
- ~~**The map draws less relief than the sim flies over (pre-existing).**~~ **RESOLVED 2026-09-21 (owner-authorized #3).** `extractTile` now serves edge-clamped partial tiles so relief reaches the DEM crop bounds; `terrainModel` widened to `cropBounds`. Original note: `scenarioTerrainLayers.impl.ts` `extractTile` served only DEM tiles lying *wholly* inside the committed crop — outside that block MapLibre's ground was flat at 0 m while the sim used real elevation. (The float was already neutralised by seating aircraft on the drawn ground; this fix draws the relief itself.)
...
## Files touched — owner-authorized fixes (2026-09-21)
- changed (scene3d): `atmosphere.ts` (#4 aerial-perspective grade), `shadowReceivers.ts` (#2 caster/receiver split), `terrainModel.ts` (#3 `cropBounds`), `index.ts` (setBuildingShadows doc), `mount.ts` (#2 building shadows on in the product)
- changed (harness): `gates/gate-6.mjs` (#1 6.1 owner-adopted layer budget; 6.6 retry), `gates/gate-3.mjs` (3.4 un-skipped), `gates/gate-p0.mjs` (P0.1c allowlist), `probe.mjs` (Chrome anti-throttling)
- **One existing app file touched (owner-authorized, outside plan scope): `src/components/scenarioTerrainLayers.impl.ts`** (#3 edge-clamped partial tiles).
- workspace-level (not this repo): `.claude/launch.json` gained a `drone-sim-harness` preview entry — harness dev-server convenience; lives in the parent workspace, not committed here.
